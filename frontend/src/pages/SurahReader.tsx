...
const SurahReader: React.FC = () => {
  ...
  // Universal Bulletproof Audio URL Resolver for all 11 Arabic Qaris & Urdu Voice
  const getAyahAudioUrl = (surahNum: number, ayahNum: number, qariId: string) => {
    ...
    const qariMap: Record<string, string> = {
      ...
      'urdu_translation': 'translations/urdu_shamshad_ali_khan_46kbps',
    };
    const folder = qariMap[qariId] || 'Alafasy_128kbps';
    return `https://everyayah.com/data/${folder}/${s3}${a3}.mp3`;
  };

  // Dedicated Urdu Translation Audio URL Resolver (Universal Urdu Voice)
  const getAyahUrduAudioUrl = (surahNum: number, ayahNum: number) => {
    const s3 = String(surahNum).padStart(3, '0');
    const a3 = String(ayahNum).padStart(3, '0');
    return `https://everyayah.com/data/translations/urdu_shamshad_ali_khan_46kbps/${s3}${a3}.mp3`;
  };
  ...
  const togglePlay = (
    ...
    if (explicitUrl && explicitUrl.startsWith('http')) {
      ...
      targetUrl = getAyahUrduAudioUrl(surah.number, ayahNumber);
    } else {
    ...
    if ('mediaSession' in navigator && surah) {
      ...
      const voiceTitle = isUrduNow
        ? 'Urdu Translation Voice (اردو ترجمہ آواز)'
  ...
  return (
    <div className="min-h-screen bg-background pb-40">
      ...
      <div className="fixed bottom-[calc(4.25rem+env(safe-area-inset-bottom,0px))] sm:bottom-20 left-2 right-2 sm:left-4 sm:right-4 z-40 max-w-md mx-auto bg-card/95 backdrop-blur-2xl border border-border/90 rounded-2xl sm:rounded-3xl shadow-2xl p-1.5 sm:p-2.5 animate-in slide-in-from-bottom-4 box-border">
        ...
        {showModeMenu && (
          <div className="absolute bottom-full left-0 right-0 mb-2 bg-card border border-border rounded-2xl sm:rounded-3xl shadow-2xl p-3 sm:p-4 animate-in slide-in-from-bottom-4 z-50">
            <div className="flex items-center justify-between mb-2.5 pb-2 border-b border-border">
              ...
              <button onClick={() => setShowModeMenu(false)} className="p-1 bg-surface rounded-full text-subtext hover:text-text">
                <X size={15} />
              </button>
            </div>
            <div className="grid grid-cols-3 gap-1.5 sm:gap-2">
              ...
              >
                ...
                <span className="text-[11px] sm:text-xs font-bold">Verses</span>
                <span className="text-[8px] sm:text-[9px] opacity-75">Verse & Meaning</span>
              </button>
              ...
              >
                <span className="text-base sm:text-lg">🔤</span>
                <span className="text-[11px] sm:text-xs font-bold">Words</span>
                <span className="text-[8px] sm:text-[9px] opacity-75">Word by Word</span>
              </button>
              ...
              >
                <span className="text-base sm:text-lg">📜</span>
                <span className="text-[11px] sm:text-xs font-bold">Mushaf</span>
                <span className="text-[8px] sm:text-[9px] opacity-75">Continuous Flow</span>
              </button>
            </div>
          </div>
        )}
        ...
        {showQariMenu && (
          <div className="absolute bottom-full left-0 right-0 mb-2 bg-card border border-border rounded-2xl sm:rounded-3xl shadow-2xl p-3 sm:p-4 max-h-[65vh] overflow-y-auto animate-in slide-in-from-bottom-4 z-50">
            <div className="flex items-center justify-between mb-2.5 pb-2 border-b border-border">
              ...
              <button onClick={() => setShowQariMenu(false)} className="p-1 bg-surface rounded-full text-subtext hover:text-text">
                <X size={15} />
              </button>
            </div>

            {/* 🌟 Urdu Voice After Arabic Recitation Setting */}
            <div className="mb-3 p-3 bg-emerald-500/10 border border-emerald-500/25 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-2">
            ...
            <div className="space-y-1">
              ...
              {QARI_OPTIONS.filter(q => q.isTranslation).map(qari => (
                ...
                >
                  <span className="text-xs font-bold">{qari.name}</span>
        ...
        {showSpeedMenu && (
          <div className="absolute bottom-full left-0 right-0 mb-2 bg-card border border-border rounded-2xl sm:rounded-3xl shadow-2xl p-3 sm:p-4 animate-in slide-in-from-bottom-4 z-50">
            <div className="flex items-center justify-between mb-2.5 pb-2 border-b border-border">
              ...
              <button onClick={() => setShowSpeedMenu(false)} className="p-1 bg-surface rounded-full text-subtext hover:text-text">
                <X size={15} />
              </button>
            </div>
            <div className="grid grid-cols-5 gap-1">
              {SPEED_OPTIONS.map(opt => (
                ...
                >
                  ...
                  <span className="text-[8px] opacity-80 block">{opt.desc}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {/* ── ULTRA-COMPACT SINGLE FLUID BAR (100% FIT ON ALL PHONE DISPLAYS) ── */}
        <div className="flex items-center justify-between gap-1 sm:gap-1.5 w-full">
          ...
          <button 
            ...
            className="flex items-center gap-0.5 sm:gap-1 py-1 px-1.5 sm:px-2 bg-primary/10 hover:bg-primary/20 rounded-xl text-[10px] sm:text-xs font-bold text-primary border border-primary/25 transition-colors shrink-0 active:scale-95"
          ...
          <button 
            ...
            className="flex items-center gap-0.5 sm:gap-1 py-1 px-1.5 sm:px-2 bg-surface hover:bg-border/60 rounded-xl text-[10px] sm:text-xs font-bold text-text border border-border/60 transition-colors max-w-[65px] sm:max-w-[95px] truncate shrink active:scale-95"
            title="Select Reciter Voice"
          >
            <Volume2 size={11} className="text-primary shrink-0" />
          ...
          <button
            ...
            className={`py-1 px-1.5 rounded-xl text-[10px] sm:text-xs font-bold border transition-colors flex items-center gap-0.5 shrink-0 active:scale-95 ${
              playbackSpeed !== 1.0
                ? 'bg-amber-500/15 border-amber-500/30 text-amber-400'
                : 'bg-surface border-border/60 text-subtext hover:text-text'
            }`}
          ...
          <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
            <button 
              onClick={playPrev}
              className="p-1 sm:p-1.5 rounded-lg bg-surface hover:bg-border text-subtext hover:text-text active:scale-90 transition-all"
              title="Previous Ayah"
            >
              <SkipBack size={13} className="sm:w-3.5 sm:h-3.5" />
            </button>
            ...
            <button 
              ...
              className="w-7 h-7 sm:w-8 sm:h-8 flex items-center justify-center bg-primary text-white rounded-xl shadow-md shadow-primary/30 hover:scale-105 active:scale-90 transition-all shrink-0"
            >
              {playingAyah !== null ? <Pause size={13} className="sm:w-3.5 sm:h-3.5" /> : <Play size={13} className="ml-0.5 sm:w-3.5 sm:h-3.5" />}
            </button>
            ...
            <button 
              ...
              title="Next Ayah"
            >
              <SkipForward size={13} className="sm:w-3.5 sm:h-3.5" />
            </button>
          </div>
